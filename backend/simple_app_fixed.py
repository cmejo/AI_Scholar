"""
Simplified FastAPI application for chat functionality - FIXED VERSION
"""
import logging
import json
import os
...
class SimpleOllamaService:
    def __init__(self):
        ...
        self.base_url = os.getenv('OLLAMA_HOST', 'http://localhost:11435')
        ...
        # Debug logging
        logger.info(f"Ollama service initialized with base_url: {self.base_url}")
    ...
    async def health_check(self):
        ...
        try:
            logger.info(f"Health check to: {self.base_url}/api/tags")
...
logger.info(f"Creating Ollama service with OLLAMA_HOST={os.getenv('OLLAMA_HOST')}")
    ...
    try:
        ...
        return {
            ...
            "environment_check": {
                ...
                "all_env_vars": {k: v for k, v in os.environ.items() if 'OLLAMA' in k}
            }
        }
    except Exception as e:
        return {

...
__all__ = [
   ...
   async def _detect_update_patterns(self, recent_updates: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Detect patterns in recent knowledge graph updates"""
        try:
            patterns = []
            ...
            for user_id, updates in user_updates.items():
                if len(updates) > 2:
                    patterns.append({
                        ...
                        "description": f"High knowledge graph activity detected",
                        "data": {"update_count": len(updates), "pattern": "activity_spike"}
                    })
            
            return patterns
            
        except Exception as e:
            logger.error(f"Error detecting update patterns: {str(e)}")
            return []

    async def get_user_notifications(self, user_id: str, limit: int = 20) -> List[SmartNotification]:
    ...
    async def get_processing_job_status(self, job_id: str) -> Optional[LiveProcessingJob]:
        """Get processing job status"""
        try:
            return self.active_jobs.get(job_id)
        except Exception as e:
            logger.error(f"Error getting processing job status: {str(e)}")
            return None
    ...
    async def cancel_processing_job(self, job_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error cancelling processing job: {str(e)}")
            return False
    ...
    async def get_real_time_metrics(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "active_connections": 1 if user_id in self.active_connections else 0,
                "pending_notifications": unread_count,
                "active_processing_jobs": len([j for j in self.active_jobs.values() if j.user_id == user_id and j.status in ["starting", "in_progress"]]),
                "subscriptions": list(self.user_subscriptions.get(user_id, set())),
                "analytics_stream_active": user_id in self.analytics_streams
            }
        except Exception as e:

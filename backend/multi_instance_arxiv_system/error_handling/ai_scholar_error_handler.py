"""
AI Scholar Error Handler with specialized error handling and recovery.

Provides comprehensive error handling for AI Scholar operations including:
...
class ErrorCategory(Enum):
    """Error categories for AI Scholar operations."""
    NETWORK = "network"
...
class AIScholarErrorHandler(MultiInstanceErrorHandler):
    """Enhanced error handler for AI Scholar operations."""
    
    def __init__(self, error_log_dir: Path, instance_name: str, processor_id: str):
        """
        Initialize AI Scholar error handler.
        ...
        # AI Scholar specific error tracking
        self.ai_scholar_stats = {
            'arxiv_api_errors': 0,
        ...
        logger.info(f"AIScholarErrorHandler initialized for instance '{instance_name}'")
    ...
    async def handle_with_retry(self,
        ...
        for attempt in range(strategy.max_attempts):
            try:
                ...
                self.ai_scholar_stats['consecutive_failures'] = 0
                self.ai_scholar_stats['last_success_time'] = datetime.now()
                ...
                last_exception = e
                self.ai_scholar_stats['consecutive_failures'] += 1
                ...
                self.log_ai_scholar_error(e, error_context, error_category, severity)
    ...
    def log_ai_scholar_error(self,
                           error: Exception,
                           context: Dict[str, Any],
                           error_category: ErrorCategory,
                           severity: ErrorSeverity = ErrorSeverity.ERROR) -> ProcessingError:
        """
        Log AI Scholar specific error with enhanced categorization.
        ...
        """
        # Update AI Scholar specific statistics
        self._update_ai_scholar_stats(error_category)
        
        # Enhance context with AI Scholar specific information
        enhanced_context = context.copy()
        enhanced_context.update({
            'error_category': error_category.value,
            'severity': severity.value,
            'consecutive_failures': self.ai_scholar_stats['consecutive_failures'],
        ...
        return self.log_instance_error(
            error, 
            enhanced_context, 
            f"ai_scholar_{error_category.value}",
            severity.value
        )
    
    def _update_ai_scholar_stats(self, error_category: ErrorCategory) -> None:
        """Update AI Scholar specific error statistics."""
        category_map = {
        ...
        if stat_key:
            self.ai_scholar_stats[stat_key] += 1
    ...
    def get_ai_scholar_error_summary(self) -> Dict[str, Any]:
        """Get comprehensive AI Scholar error summary."""
        base_summary = self.get_instance_error_summary()
        
        return {
            **base_summary,
            'ai_scholar_stats': self.ai_scholar_stats.copy(),
    ...
    def _get_health_status(self) -> str:
        ...
        total_errors = self.ai_scholar_stats['consecutive_failures']
    ...
    def _get_error_recommendations(self) -> List[str]:
        ...
        if self.ai_scholar_stats['consecutive_failures'] >= 5:
    ...
    def should_continue_processing(self) -> bool:
        ...
        # Check circuit breakers
        critical_breakers = ['arxiv_api', 'vector_store']
        ...
        # Check consecutive failures
        if self.ai_scholar_stats['consecutive_failures'] >= 15:
            logger.error(f"Too many consecutive failures: {self.ai_scholar_stats['consecutive_failures']}")
            return False
        
        # Use parent method for additional checks
        return self.should_instance_continue_processing(
            max_error_rate=30.0,  # Allow 30% error rate for AI Scholar
            max_critical_errors=3
        )
    
    def export_ai_scholar_report(self, output_path: Path) -> bool:
        """Export comprehensive AI Scholar error report."""
        try:
            summary = self.get_ai_scholar_error_summary()
            
            # Add timestamp and metadata
            report = {
                'report_type': 'ai_scholar_error_report',
            ...
            logger.info(f"AI Scholar error report exported to {output_path}")
            return True
            
        except Exception as e:
            logger.error(f"Failed to export AI Scholar error report: {e}")
            return False